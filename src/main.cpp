...
//     char const* const* instanceExtensions{ SDL_Vulkan_GetInstanceExtensions(
...
//                                     .vkGetDeviceProcAddr =
//                                     vkGetDeviceProcAddr, .vkCreateImage =
//                                     vkCreateImage };
...
//         "How to Vulkan", 1280u, 720u, SDL_WINDOW_VULKAN |
...
//     VkBufferCreateInfo bufferCI{ .sType =
...
//         chk(vmaMapMemory(allocator, shaderDataBuffers[i].allocation,
...
//         ktxTexture_CreateFromNamedFile(filename.c_str(),
...
//             .initialLayout = VK_IMAGE_LAYOUT_UNDEFINED
//         };
//         VmaAllocationCreateInfo texImageAllocCI{ .usage =
//                                                      VMA_MEMORY_USAGE_AUTO };
//         chk(vmaCreateImage(allocator, &texImgCI, &texImageAllocCI,
//                            &textures[i].image, &textures[i].allocation,
//                            nullptr));
...
//                                   .layerCount = 1 }
//         };
//         chk(vkCreateImageView(device, &texVewCI, nullptr,
//         &textures[i].view));
//         // Upload
...
//                             &imgSrcBuffer, &imgSrcAllocation, nullptr));
...
//                                   .layerCount = 1 }
//         };
//         VkDependencyInfo barrierTexInfo{ .sType =
...
//         };
//         chk(vkCreateSampler(device, &samplerCI, nullptr,
//         &textures[i].sampler)); ktxTexture_Destroy(ktxTexture);
...
//         .pPoolSizes = &poolSize
//     };
//     chk(vkCreateDescriptorPool(device, &descPoolCI, nullptr,
//     &descriptorPool)); uint32_t variableDescCount{
...
//     };
//     chk(vkAllocateDescriptorSets(device, &texDescSetAlloc,
//     &descriptorSetTex)); VkWriteDescriptorSet writeDescSet{
...
//     slangGlobalSession->createSession(slangSessionDesc,
//                                       slangSession.writeRef());
//     Slang::ComPtr<slang::IModule> slangModule{
//         slangSession->loadModuleFromSource("triangle", "assets/shader.slang",
//                                            nullptr, nullptr)
//     };
...
//     };
//     chk(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineCI,
//                                   nullptr, &pipeline));
//     // Render loop
...
//     {
//         // Sync
//         chk(vkWaitForFences(device, 1, &fences[frameIndex], true,
//         UINT64_MAX)); chk(vkResetFences(device, 1, &fences[frameIndex]));
...
//             shaderData.model[i] = glm::translate(glm::mat4(1.0f),
//             instancePos) *
//                                   glm::mat4_cast(glm::quat(objectRotations[i]));
//         }
//         memcpy(shaderDataBuffers[frameIndex].mapped, &shaderData,
//                sizeof(ShaderData));
...
//                 .sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2,
//                 .srcStageMask =
//                 VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, .srcAccessMask
//                 = 0, .dstStageMask =
//                 VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, .dstAccessMask
...
//                                 VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
//                 .dstAccessMask =
//                 VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT, .oldLayout =
//                 VK_IMAGE_LAYOUT_UNDEFINED, .newLayout =
//                 VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL, .image = depthImage,
//                 .subresourceRange{ .aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT |
...
//             {
//                 if (event.key.key == SDLK_PLUS || event.key.key ==
//                 SDLK_KP_PLUS)
//                 {
//                     shaderData.selected =
//                         (shaderData.selected < 2) ? shaderData.selected + 1 :
//                         0;
//                 }
...
//                 {
//                     shaderData.selected =
//                         (shaderData.selected > 0) ? shaderData.selected - 1 :
//                         2;
//                 }
//             }
//             // Window resize
...
//                 devices[deviceIndex], surface, &surfaceCaps));
//             swapchainCI.oldSwapchain = swapchain;
//             swapchainCI.imageExtent = {
//                 .width = static_cast<uint32_t>(windowSize.x),
//                 .height = static_cast<uint32_t>(windowSize.y)
//             };
//             chk(vkCreateSwapchainKHR(device, &swapchainCI, nullptr,
//                                      &swapchain));
//             for (auto i = 0; i < imageCount; i++)
//             {
...
//         vmaDestroyImage(allocator, textures[i].image,
